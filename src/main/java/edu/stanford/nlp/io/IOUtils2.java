...
public class IOUtils2 {
  ...
  private static InputStream findStreamInClasspathOrFileSystem(String name) throws FileNotFoundException {
    ...
    InputStream is = null;
    if (path != null) {
        is = IOUtils2.class.getClassLoader().getResourceAsStream(path);
        // windows File.separator is \, but getting resources only works with /
        if (is == null) {
           is = IOUtils2.class.getClassLoader().getResourceAsStream(path.replaceAll("\\\\", "/"));
        }
    }
  ...
  public static String slurpFile(File file, String encoding) throws IOException {
    return IOUtils2.slurpReader(IOUtils2.encodedInputStreamReader(
  ...
  public static String slurpGBFileNoExceptions(String filename) {
    return IOUtils2.slurpFileNoExceptions(filename, "GB18030");
  }
  ...
  public static String slurpFile(String filename) throws IOException {
    return IOUtils2.slurpReader(new FileReader(filename));
  }
  ...
  public static String slurpGBURL(URL u) throws IOException {
    return IOUtils2.slurpURL(u, "GB18030");
  }
  ...
  public static String slurpURLNoExceptions(URL u, String encoding) {
    try {
      return IOUtils2.slurpURL(u, encoding);
    } catch (Exception e) {
      e.printStackTrace();
      return null;
    }
  ...
  public static String slurpFileNoExceptions(File file) {
    try {
      return IOUtils2.slurpReader(new FileReader(file));
    } catch (Exception e) {
      e.printStackTrace();
      return null;
    }
  ...
  public static List<Map<String,String>> readCSVWithHeader(String path, char quoteChar, char escapeChar) throws IOException {
    ...
    for (String line : IOUtils2.readLines(path)) {
  ...
  {
    BufferedReader br = IOUtils2.getBufferedFileReader(infile);
    String line;
          ...
          NoSuchFieldException, NoSuchMethodException, InvocationTargetException
  {
    ...
    BufferedReader br = IOUtils2.getBufferedFileReader(filename);
    String line;
    while ((line = br.readLine()) != null) {
      line = line.trim();
      if (line.length() > 0) {
